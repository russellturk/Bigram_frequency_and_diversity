...
model_a <- brm(response_time ~ age + concreteness + letters + word_freq + trans_prob + diversity
               + (1|subject_nr) + (1|item), data = df, prior = priors_div_tp, save_all_pars = TRUE)

model_b <- brm(response_time ~ age + concreteness + letters + word_freq + diversity
               + (1|subject_nr) + (1|item), data = df, prior = priors_div, save_all_pars = TRUE)
